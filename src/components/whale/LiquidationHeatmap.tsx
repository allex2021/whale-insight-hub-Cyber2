...
export function LiquidationHeatmap() {
  ...
  const totals = useMemo(() => {
    ...
    const shorts = data.reduce((s, d) => s + d.shortLiq, 0);
    const top = [...data].sort((a, b) => (b.longLiq + b.shortLiq) - (a.longLiq + a.shortLiq))[0];
    return { longs, shorts, top };
  }, [data]);

  const cascadeScore = Math.min(95, Math.round((totals.longs + totals.shorts) / 4e8));
  ...
  return (
    <Panel title="Liquidation Heatmap" subtitle="BTC liquidatable size ±15% from spot" accent="orange">
      ...
      <div className="h-[260px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} stackOffset="sign" margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            />
            <ReferenceLine x={current} stroke="white" strokeDasharray="4 4" label={{ value: "Spot", fill: "white", fontSize: 10, position: "top" }} />
